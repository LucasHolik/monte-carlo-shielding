...
SourceGeometry SourceGeometry::createPlane(const Vector3D &center,
                                           const Vector3D &normal, double width,
                                           double height)
{
  SourceGeometry geom;
...
SourceGeometry SourceGeometry::createSphere(const Vector3D &center,
                                            double radius, bool surface_only)
{
  SourceGeometry geom;
...
SourceGeometry SourceGeometry::createCylinder(const Vector3D &center,
                                              const Vector3D &axis,
...
SourceGeometry SourceGeometry::createRectangle(const Vector3D &corner,
                                               const Vector3D &side1,
                                               const Vector3D &side2)
{
  ...
  geom.parameter3 =
      side1.dot(side2) / (side1.magnitude() * side2.magnitude()); // cos(angle)
  return geom;
}

SourceGeometry SourceGeometry::createCone(const Vector3D &apex,
                                          const Vector3D &axis,
...
IntensityDistribution::createGaussian(const Vector3D &center, double sigma)
{
  IntensityDistribution dist;
...
IntensityDistribution
IntensityDistribution::createExponential(const Vector3D &center, double lambda)
{
  IntensityDistribution dist;
...
bool SourceConfiguration::isValid() const
{
  ...
  if(energy_spectrum.min_energy < 0.0 ||
     energy_spectrum.max_energy < energy_spectrum.min_energy)
  {
    return false;
  }
  ...
  case SourceGeometryType::Sphere:
    if(geometry.parameter1 <= 0.0)
      return false;
    break;
  case SourceGeometryType::Cylinder:
    if(geometry.parameter1 <= 0.0 || geometry.parameter2 <= 0.0)
      return false;
    break;
...
std::string SourceConfiguration::toString() const
{
  ...
  ss << "  Energy Range: " << energy_spectrum.min_energy << " - "
     << energy_spectrum.max_energy << " MeV\n";
  ss << "  Position: (" << geometry.center.x() << ", " << geometry.center.y()
     << ", " << geometry.center.z() << ")\n";
  return ss.str();
}
...
SourceParticle ParticleSource::generateParticle()
{
  ...
  source_particle.statistical_weight =
      config_.base_statistical_weight * intensity_weight;
...
std::vector<SourceParticle>
ParticleSource::generateParticlesForTime(double time_duration)
{
  std::vector<SourceParticle> particles;
  ...
  size_t estimated_count =
      static_cast<size_t>(config_.activity * time_duration * 1.1); // 10% buffer
...
std::vector<SourceParticle> ParticleSource::generateParticlesBatch(
    size_t count, std::function<void(size_t, size_t)> progress_callback)
{
  std::vector<SourceParticle> particles;
  particles.reserve(count);
...
double ParticleSource::sampleEmissionTime() const
{
  ...
  return current_simulation_time_ +
         sampling_.getRNG().uniform(0.0, 1.0 / config_.activity);
}
...
Vector3D ParticleSource::sampleSphereSourcePosition() const
{
  ...
  else
  {
    ...
    double r =
        radius * std::pow(sampling_.getRNG().uniform(0.0, 1.0), 1.0 / 3.0);
    Vector3D direction = sampling_.sampleIsotropicDirection();
...
Vector3D ParticleSource::sampleCylinderSourcePosition() const
{
  ...
  if(surface_only)
  {
    ...
    double total_area =
        2.0 * M_PI * radius * height + 2.0 * M_PI * radius * radius;
    ...
    else
    {
      ...
      double phi = sampling_.getRNG().uniform(0.0, 2.0 * M_PI);
      double z = sampling_.getRNG().uniform(0.0, 1.0) > 0.5 ? height / 2.0
                                                            : -height / 2.0;
...
SourceConfiguration ParticleSource::createMonoenergeticPointSource(
    const Vector3D &position, double energy_MeV, ParticleType type)
{
...
SourceConfiguration ParticleSource::createIsotropicPointSource(
    const Vector3D &position, const EnergySpectrum &spectrum, ParticleType type)
{
  SourceConfiguration config;
                                                     ...
                                                     ParticleType type)
{
  ...
  {
    config.angular_distribution =
        AngularDistribution::cone(direction, beam_divergence);
  }
  else
  {
...
bool validateSourceGeometry(const SourceGeometry &geometry)
{
  ...
  case SourceGeometryType::Cylinder:
    return geometry.parameter1 > 0.0 &&
           geometry.parameter2 > 0.0; // Radius and height
  case SourceGeometryType::Plane:
    return geometry.parameter1 > 0.0 &&
           geometry.parameter2 > 0.0; // Width and height
  default:
    return true;
  }
}
...
std::string generateQualityAssuranceReport(const SourceConfiguration &config)
{
  ...
  ss << "  Configuration Valid: " << (config.isValid() ? "PASS" : "FAIL")
     << "\n";
  ss << "  Geometry Valid: "
     << (validateSourceGeometry(config.geometry) ? "PASS" : "FAIL") << "\n";
  ss << "  Estimated Memory: " << estimateMemoryRequirement(config) << " MB\n";
  ss << "  Effective Area: " << calculateEffectiveSourceArea(config.geometry)
     << " cm²\n";
  return ss.str();
}
