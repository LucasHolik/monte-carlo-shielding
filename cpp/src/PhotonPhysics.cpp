...
PhotonPhysics::performInteraction(const Particle &photon,
...
PhotonInteractionType
PhotonPhysics::sampleInteractionType(double energy, const Material &material)
{
                                              ...
                                              const Material &material)
{
  PhotonInteractionResult result(
      PhotonInteractionType::PHOTOELECTRIC_ABSORPTION);
...
PhotonInteractionResult
PhotonPhysics::performComptonScattering(const Particle &photon,
                                        const Material &material)
{
  ...
  Vector3D new_direction =
      sampleScatteredDirection(photon.direction(), cos_theta, phi);
...
PhotonInteractionResult
PhotonPhysics::performCoherentScattering(const Particle &photon,
                                         const Material &material)
{
...
PhotonInteractionResult
PhotonPhysics::performPairProduction(const Particle &photon,
                                     const Material &material)
{
  PhotonInteractionResult result(
      PhotonInteractionType::PAIR_PRODUCTION_NUCLEAR);
...
double PhotonPhysics::getPhotoelectricCrossSection(double energy,
                                                   int atomic_number) const
{
  ...
  double cross_section_barns = PhotonCrossSections::ScofieldPhotoelectricData::
      getPhotoelectricCrossSection(atomic_number, energy_keV);

  // Convert from barns to cm²
  return PhotonCrossSections::PhotonInteractionDatabase::barnsToCm2(
      cross_section_barns);
}

double PhotonPhysics::getComptonCrossSection(double energy,
                                             int atomic_number) const
{
  // Convert energy from MeV to keV for the database
  double energy_keV = energy * 1000.0;
  ...
  double cross_section_barns = PhotonCrossSections::IncoherentScatteringData::
      getTotalIncoherentCrossSection(atomic_number, energy_keV);
...
double PhotonPhysics::getCoherentCrossSection(double energy,
                                              int atomic_number) const
{
  // Convert energy from MeV to keV for the database
  double energy_keV = energy * 1000.0;
  ...
  double cross_section_barns =
      PhotonCrossSections::CoherentScatteringData::getCoherentCrossSection(
...
double PhotonPhysics::getPairProductionCrossSection(double energy,
                                                    int atomic_number) const
{
  if(!isPairProductionPossible(energy))
  {
  ...
  double cross_section_barns = PhotonCrossSections::PairProductionData::
      getTotalPairProductionCrossSection(atomic_number, energy);
...
double PhotonPhysics::getTotalCrossSection(double energy,
...
double PhotonPhysics::kleinNishinaCrossSection(double energy) const
{
  ...
  // σ = 2πr_e² [(1+α)/α³] [(2α(1+α))/(1+2α) - ln(1+2α)] + ln(1+2α)/(2α) -
  // (1+3α)/(1+2α)²
...
double PhotonPhysics::kleinNishinaDifferential(double energy,
...
Vector3D
PhotonPhysics::sampleScatteredDirection(const Vector3D &incident_direction,
                                        double cos_theta, double phi)
{
  return rotateDirection(incident_direction, cos_theta, phi);
}

bool PhotonPhysics::isPhotoelectricPossible(double energy,
                                            const Material &material) const
{
...
double PhotonPhysics::calculateMassAttenuationCoefficient(
    double energy, const Material &material) const
{
  double total_cross_section = getTotalCrossSection(energy, material);

  // Convert to mass attenuation coefficient
  // Note: This is a simplified conversion - full implementation would account
  // for proper atomic mass and Avogadro's number scaling
  return total_cross_section / material.density();
}

bool PhotonPhysics::validateInteractionResult(
...
std::string
PhotonPhysics::getInteractionTypeName(PhotonInteractionType type) const
{
  switch(type)
  {
  case PhotonInteractionType::PHOTOELECTRIC_ABSORPTION:
...
double PhotonPhysics::approximatePhotoelectricCrossSection(double energy,
                                                           int Z) const
{
  if(energy <= 0.0)
    return 0.0;
...
double PhotonPhysics::approximateCoherentCrossSection(double energy,
                                                      int Z) const
{
  if(energy <= 0.0)
    return 0.0;
...
double PhotonPhysics::approximatePairProductionCrossSection(double energy,
                                                            int Z) const
{
  if(energy < PAIR_PRODUCTION_THRESHOLD)
    return 0.0;
...
Vector3D PhotonPhysics::rotateDirection(const Vector3D &original,
                                        double cos_theta, double phi)
{
...
double PhotonPhysics::calculatePhotoelectricCoefficient(double energy,
                                                        int Z) const
{
...
double PhotonPhysics::calculatePairProductionCoefficient(double energy,
                                                         int Z) const
{
