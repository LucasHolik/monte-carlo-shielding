...
bool SimulationConfiguration::isValid() const
{
  if(number_of_histories == 0)
    return false;
  if(energy_cutoff_keV < 0.0)
    return false;
  if(time_cutoff_seconds <= 0.0)
    return false;
  if(batch_size == 0 || batch_size > number_of_histories)
    return false;
  if(target_statistical_uncertainty <= 0.0 ||
     target_statistical_uncertainty > 1.0)
    return false;
...
void PhotonSimulation::setGeometry(std::shared_ptr<Geometry> geometry)
{
  if(geometry && sampling_)
  {
    ...
    transport_->enableDetailedResultsCollection(
        config_.enable_detailed_logging);
  }
}
...
std::shared_ptr<SimulationResults> PhotonSimulation::runSimulation()
{
  ...
  try
  {
          ...
          !checkTerminationCriteria() && is_running_)
    {
      ...
      size_t remaining_histories =
          config_.number_of_histories - histories_completed_;
      size_t current_batch_size =
          std::min(config_.batch_size, remaining_histories);
...
std::shared_ptr<SimulationResults>
PhotonSimulation::runSimulation(const SourceConfiguration &source_config,
...
void PhotonSimulation::runSingleBatch(size_t batch_size)
{
  ...
  for(size_t i = 0; i < batch_size && is_running_; ++i)
  {
       ...
       (histories_completed_ % config_.progress_reporting_frequency == 0))
    {
      ...
      config_.progress_callback(histories_completed_,
                                config_.number_of_histories, elapsed_time);
    }
  }
...
ParticleHistory
PhotonSimulation::transportSingleParticle(const SourceParticle &source_particle)
{
  ...
  while(particle.isAlive())
  {
    ...
    history.total_path_length +=
        (particle.position() - position_before).magnitude();
...
void PhotonSimulation::processParticleHistory(const ParticleHistory &history)
{
  ...
  if(history.final_state == ParticleState::Escaped)
  {
    ...
    if(history.final_position.z() > history.initial_particle.position().z())
    {
      ...
      // results_->transmission_coefficient_.add(weight); // Would need to
      // expose this
    }
  }
}
...
{
  if(config_.number_of_histories == 0)
    return 0.0;
...
std::string PhotonSimulation::getStatusSummary() const
{
  ...
  ss << "(" << histories_completed_ << "/" << config_.number_of_histories
     << " histories)\n";
  ss << "  Elapsed time: " << getElapsedTime() << " seconds\n";
  ss << "  Rate: " << calculateParticlesPerSecond() << " particles/second\n";
  if(histories_completed_ > 0)
  {
    ss << "  Estimated remaining: " << getEstimatedRemainingTime()
       << " seconds\n";
  }
...
bool PhotonSimulation::validateParticleHistory(
    const ParticleHistory &history) const
{
  ...
  double relative_error =
      std::abs(total_final - initial_energy) / initial_energy;
...
std::shared_ptr<SimulationResults>
runPhotonSimulation(const SourceConfiguration &source_config,
                    std::shared_ptr<Geometry> geometry, size_t num_histories,
                    std::function<void(size_t, size_t)> progress_callback)
{
  ...
  {
    config.progress_callback = [progress_callback](size_t completed,
                                                   size_t total, double time) {
...
ShieldingAnalysisResult analyzeShielding(const Vector3D &source_position,
                                         ...
                                         size_t num_histories)
{
  ...
  analysis.transmission_coefficient =
      static_cast<double>(escaped) / num_histories;
                                 ...
                                 const SimulationResults &pilot_results)
{
  ...
  return static_cast<size_t>(pilot_results.getTotalHistoriesCompleted() *
                             ratio * ratio);
}
...
bool validateSimulationPhysics(const SimulationResults &results,
                               double tolerance)
{
  // Basic validation checks
  if(results.getTotalEnergyDeposited() < 0.0)
    return false;
  if(results.getTotalHistoriesCompleted() == 0)
    return false;
                              ...
                              double simulation_time_seconds)
{
  ...
  if(relative_error <= 0.0 || simulation_time_seconds <= 0.0)
    return 0.0;
