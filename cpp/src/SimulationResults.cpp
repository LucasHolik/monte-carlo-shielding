...
{
  if (total_histories_requested_ == 0) return 0.0;
  return 100.0 * total_histories_completed_ / total_histories_requested_;
}

void SimulationResults::recordEnergyDeposition(const std::string& material_name,
                                             double energy_deposited,
                                             PhotonInteractionType interaction_type)
{
  if (energy_deposited <= 0.0) return;
...
void SimulationResults::recordPerformanceData(const PerformanceData& perf_data)
{
  ...
  if (performance_.simulation_time_seconds > 0.0) {
    performance_.particles_per_second = total_histories_completed_ / performance_.simulation_time_seconds;
  }
}

EnergyDepositionData SimulationResults::getEnergyDepositionForMaterial(const std::string& material_name) const
{
  auto it = energy_deposition_by_material_.find(material_name);
  return (it != energy_deposition_by_material_.end()) ? it->second : EnergyDepositionData{};
}

FluxData SimulationResults::getFluxForRegion(const std::string& region_name) const
{
...
bool SimulationResults::hasConverged(double target_relative_error) const
{
  ...
  bool total_energy_converged = total_energy_deposited_.relativeError() < target_relative_error;
  bool transmission_converged = true;
  bool reflection_converged = true;
  
  if (transmission_coefficient_.count > 0) {
    transmission_converged = transmission_coefficient_.relativeError() < target_relative_error;
  }
  
  if (reflection_coefficient_.count > 0) {
    reflection_converged = reflection_coefficient_.relativeError() < target_relative_error;
  }
  
  return total_energy_converged && transmission_converged && reflection_converged;
}

std::vector<std::string> SimulationResults::getConvergenceSummary(double target_relative_error) const
{
  std::vector<std::string> summary;
  
  double total_energy_error = total_energy_deposited_.relativeError();
  summary.push_back("Total Energy Deposition: " + 
    std::to_string(total_energy_error * 100.0) + "% " +
    (total_energy_error < target_relative_error ? "(CONVERGED)" : "(NOT CONVERGED)"));
  
  if (transmission_coefficient_.count > 0) {
    double trans_error = transmission_coefficient_.relativeError();
    summary.push_back("Transmission Coefficient: " +
      std::to_string(trans_error * 100.0) + "% " +
      (trans_error < target_relative_error ? "(CONVERGED)" : "(NOT CONVERGED)"));
  }
  
  if (reflection_coefficient_.count > 0) {
    double refl_error = reflection_coefficient_.relativeError();
    summary.push_back("Reflection Coefficient: " +
      std::to_string(refl_error * 100.0) + "% " +
      (refl_error < target_relative_error ? "(CONVERGED)" : "(NOT CONVERGED)"));
  }
...
std::string SimulationResults::getSummaryReport() const
{
  ...
  if (!energy_deposition_by_material_.empty()) {
    ...
    for (const auto& [material_name, data] : energy_deposition_by_material_) {
      ss << "  " << material_name << ": " 
         << data.total_energy_deposited.mean() << " ± " 
  ...
  if (transmission_coefficient_.count > 0) {
    ...
    ss << "  Relative Error: " << transmission_coefficient_.relativeError() * 100.0 << "%\n";
  ...
  ss << "  Simulation Time: " << performance_.simulation_time_seconds << " seconds\n";
...
void SimulationResults::merge(const SimulationResults& other)
{
  ...
  for (const auto& [material_name, other_data] : other.energy_deposition_by_material_) {
    ...
    for (const auto& [interaction_type, other_tally] : other_data.by_interaction_type) {
  ...
  for (const auto& [interaction_type, count] : other.performance_.interactions_by_type) {
...
void SimulationResults::exportToCSV(const std::string& filename) const
{
  ...
  file << "Material,Total_Energy_Deposited_MeV,Energy_Uncertainty_MeV,Relative_Error_Percent\n";
  for (const auto& [material_name, data] : energy_deposition_by_material_) {
    file << material_name << ","
         << data.total_energy_deposited.mean() << ","
  ...
  file << "\n# Flux Data\n";
  file << "Region,Particle_Count,Fluence_per_cm2,Fluence_Uncertainty,Energy_Flux_MeV_per_cm2\n";
  for (const auto& [region_name, data] : flux_by_region_) {
    file << region_name << ","
         << data.particle_count.sum << ","
         << data.fluence.mean() << ","
         << data.fluence.standardError() << ","
...
{
  if (mass_g <= 0.0 || time_s <= 0.0) return 0.0;
...
{
  if (transmission_coefficient <= 0.0 || transmission_coefficient > 1.0 || thickness_cm <= 0.0) {
    return 0.0;
  }
...
{
  if (uncollided_flux <= 0.0) return 1.0;
  return total_flux / uncollided_flux;
}

bool validateEnergyConservation(const SimulationResults& results, double tolerance)
{
  ...
  // Basic sanity checks
  if (total_deposited < 0.0) return false;
  if (std::isnan(total_deposited) || std::isinf(total_deposited)) return false;
