...
Transport::Transport(const Geometry &geometry, MonteCarloSampling &sampling)
    : geometry_(geometry), sampling_(sampling),
      photon_physics_(
          std::make_unique<PhotonPhysics>(sampling)), // Enable photon physics
      maximum_step_size_(1e30), enable_boundary_crossing_(true),
      enable_interaction_sampling_(true), results_(nullptr),
      collect_detailed_results_(false)
{
...
TransportStep Transport::transportParticleStep(Particle &particle)
{
  ...
  if(hitBoundary(particle, distance))
  {
    ...
    step.boundary_info = findBoundaryAlongRay(particle.position(),
                                              particle.direction(), distance);
...
bool Transport::hitBoundary(const Particle &particle, double proposed_distance)
{
  ...
  // Use ray-tracing to check if boundary would be crossed within proposed
  // distance
  auto boundary = findBoundaryAlongRay(particle.position(),
                                       particle.direction(), proposed_distance);
...
PhotonInteractionResult Transport::performInteraction(Particle &particle,
                                                      const Material &material)
{
  ...
  else
  {
    ...
    if(results_)
    {
      recordEnergyDeposition(material, particle.energy(),
...
std::optional<GeometryIntersection>
Transport::findBoundaryAlongRay(const Vector3D &start_pos,
...
void Transport::handleBoundaryCrossing(Particle &particle,
                                       double sampled_distance,
                                       const Material &current_material)
{
  // Ray-tracing approach: Don't move particle yet, calculate intended step
  // first
  ...
  auto boundary_intersection =
      findBoundaryAlongRay(current_position, direction, sampled_distance);

  if(!boundary_intersection)
  {
    // No boundary crossed - this shouldn't happen if we're in
    // handleBoundaryCrossing Fall back to normal step
  ...
  // Get material B properties (peek slightly into new material for material
  // query)
  const double TINY_EPSILON = 1e-12;
  Vector3D peek_position =
      boundary_intersection->intersection_point + direction * TINY_EPSILON;
  ...
  double mu_current = calculateLinearAttenuationCoefficient(current_material,
                                                            particle.energy());
  double mu_new =
      calculateLinearAttenuationCoefficient(new_material, particle.energy());
  ...
  double rescaled_distance_in_new_material =
      remaining_distance * (mu_current / mu_new);

  // Move particle directly to final position in single step
  Vector3D final_position = boundary_intersection->intersection_point +
                            direction * rescaled_distance_in_new_material;
...
void Transport::handleSecondaryParticles(
    const std::vector<Particle> &secondaries)
{
...
void Transport::recordInteraction(const Particle &particle,
                                  const Material &material,
                                  const PhotonInteractionResult &result)
{
  ...
  if(collect_detailed_results_)
  {
    InteractionRecord record =
        createInteractionRecord(particle, material, result);
    results_->recordInteraction(record);
  }
}

void Transport::recordEnergyDeposition(const Material &material,
                                       double energy_deposited,
                                       PhotonInteractionType interaction_type)
{
  if(!results_ || energy_deposited <= 0.0)
    return;

  // Record in simulation results
  results_->recordEnergyDeposition(std::string(material.name()),
                                   energy_deposited, interaction_type);
}

InteractionRecord
Transport::createInteractionRecord(const Particle &particle,
                                   const Material &material,
