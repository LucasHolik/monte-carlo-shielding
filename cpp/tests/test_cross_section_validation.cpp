...
private:
  void testPhotoelectricAccuracy()
  {
    ...
    for(const auto &test_case : test_cases)
    {
      ...
      double xs_barns = PhotonCrossSections::ScofieldPhotoelectricData::
          getPhotoelectricCrossSection(Z_lead, energy_keV);

      std::cout << "Lead photoelectric @ " << energy_keV << " keV: " << xs_barns
                << " barns" << std::endl;
    ...
    double xs_below_K = PhotonCrossSections::ScofieldPhotoelectricData::
        getPhotoelectricCrossSection(Z_lead, 50.0);
    double xs_above_K = PhotonCrossSections::ScofieldPhotoelectricData::
        getPhotoelectricCrossSection(Z_lead, 100.0);

    std::cout << "Lead photoelectric below K-edge (50 keV): " << xs_below_K
              << " barns" << std::endl;
    std::cout << "Lead photoelectric above K-edge (100 keV): " << xs_above_K
              << " barns" << std::endl;
  ...
  void testCoherentScattering()
  {
    ...
    for(int Z : elements)
    {
      ...
      std::cout << "Z=" << Z << " coherent @ " << energy
                << " keV: " << xs_coherent << " barns" << std::endl;
      ...
      {
        std::cout
            << "  ⚠ Warning: Zero coherent scattering (needs investigation)"
            << std::endl;
      }
    }
  ...
  void testPairProductionThresholds()
  {
    ...
    double threshold_MeV =
        PhotonCrossSections::Constants::PAIR_PRODUCTION_THRESHOLD_MEV;
    std::cout << "Pair production threshold: " << threshold_MeV << " MeV"
              << std::endl;
    ...
    for(double energy : energies)
    {
      ...
      bool can_occur =
          PhotonCrossSections::PairProductionData::isPairProductionPossible(
              energy);

      assert(should_occur == can_occur);

      double xs_pair = PhotonCrossSections::PairProductionData::
          getTotalPairProductionCrossSection(Z, energy);

      std::cout << "Z=" << Z << " pair production @ " << energy
                << " MeV: " << xs_pair << " barns";
      ...
      if(should_occur)
      {
        ...
        {
          std::cout << " ⚠ Warning: Expected non-zero but got zero"
                    << std::endl;
        }
      }
      else
  ...
  void testEnergyDependence()
  {
    ...
    for(double energy : energies)
    {
      double pe_xs = PhotonCrossSections::ScofieldPhotoelectricData::
          getPhotoelectricCrossSection(Z, energy);
      double compton_xs = PhotonCrossSections::IncoherentScatteringData::
          getTotalIncoherentCrossSection(Z, energy);
      double coherent_xs =
          PhotonCrossSections::CoherentScatteringData::getCoherentCrossSection(
              Z, energy);
