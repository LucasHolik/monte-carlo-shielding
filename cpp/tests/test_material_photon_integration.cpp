...
{
  std::cout
      << "=== Testing Material-PhotonCrossSectionDatabase Integration ===\n";
  ...
  std::cout << "  Difference: " << std::abs(manual_total_xs - water_total_xs)
            << " barns\n";
  ...
  for(double energy : energies)
  {
    ...
    std::cout << "  " << std::setw(11) << std::fixed << std::setprecision(0)
              << energy << " | " << std::setw(15) << std::scientific
              << std::setprecision(3) << lin_mu << " | " << std::setw(14)
              << mass_mu << "\n";
  }

  std::cout << std::scientific << std::setprecision(6);
  std::cout << "\n✅ All Material-PhotonCrossSectionDatabase integration tests "
               "passed!\n";
