...
private:
  void testKleinNishinaCrossSection()
  {
    ...
    double relative_error =
        std::abs(thomson_xs - expected_thomson) / expected_thomson;
  ...
  void testPhotoelectricCrossSection()
  {
    ...
    {
      double xs1 =
          physics_.getPhotoelectricCrossSection(energy, atomic_numbers[i - 1]);
      double xs2 =
          physics_.getPhotoelectricCrossSection(energy, atomic_numbers[i]);
  ...
  void testComptonScattering()
  {
    ...
    assert(result.interaction_type ==
           PhotonInteractionType::COMPTON_SCATTERING);
    assert(result.particle_scattered == true);
    assert(result.particle_absorbed == false);
    assert(result.new_energy > 0.0);
    assert(result.new_energy <=
           energy); // Scattered energy should be less or equal
  ...
  void testCrossSectionCalculations()
  {
    ...
    double photoelectric_xs =
        physics_.getPhotoelectricCrossSection(energy, 1) * 0.5 +
  ...
  void testInteractionSampling()
  {
    ...
    aluminum.addElement(13, 26.98, 1.0, "Al");
    double energy =
        1.0; // MeV - higher energy to get multiple interaction types
