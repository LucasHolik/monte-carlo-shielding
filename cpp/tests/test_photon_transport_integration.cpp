...
/**
 * @brief Test complete photon transport integration with accurate cross-sections
 */
int main() {
    ...
    for(double energy_MeV : energies_MeV) {
        ...
        std::cout << std::setw(12) << std::fixed << std::setprecision(2) << energy_MeV
                  << " | " << std::setw(16) << std::scientific << std::setprecision(3) << linear_mu
                  << " | " << std::setw(26) << std::fixed << std::setprecision(4) << transmission << "\n";
    }
    ...
    double transport_xs = DefaultCrossSections::photonTotalCrossSection(lead, test_energy_MeV);
    
    // Get cross-section directly from Material
    double material_xs = lead.getLinearAttenuationCoefficient(test_energy_MeV * 1000.0);
    
    std::cout << "  Test energy: " << test_energy_MeV << " MeV (" << test_energy_MeV * 1000.0 << " keV)\n";
    std::cout << "  Transport function result: " << transport_xs << " cm⁻¹\n";
    std::cout << "  Material method result:    " << material_xs << " cm⁻¹\n";
    std::cout << "  Difference: " << std::abs(transport_xs - material_xs) << " cm⁻¹\n";
    ...
    std::cout << "Material     | Density (g/cm³) | Linear μ (cm⁻¹) | Mass μ (cm²/g)\n";
    std::cout << "-------------|-----------------|------------------|-----------------\n";
    
    for(const auto& material : materials) {
        double density = material.density();
        double linear_mu = DefaultCrossSections::photonTotalCrossSection(material, test_energy);
        double mass_mu = material.getMassAttenuationCoefficient(test_energy * 1000.0);
        
        std::cout << std::left << std::setw(12) << material.name()
                  << " | " << std::setw(15) << std::fixed << std::setprecision(3) << density
                  << " | " << std::setw(16) << std::scientific << std::setprecision(3) << linear_mu
                  << " | " << std::setw(15) << std::scientific << std::setprecision(3) << mass_mu << "\n";
    }
    ...
    std::cout << "Energy (keV) | Total XS (barns) | Linear μ (cm⁻¹) | Photoelectric | Compton | Coherent\n";
    std::cout << "-------------|-------------------|------------------| -------------|---------|----------\n";
    ...
    for(double energy_keV : test_energies_keV) {
        ...
        std::cout << std::setw(12) << std::fixed << std::setprecision(0) << energy_keV
                  << " | " << std::setw(17) << std::scientific << std::setprecision(2) << total_xs_barns
                  << " | " << std::setw(16) << linear_mu
                  << " | " << std::setw(13) << components[0] // Photoelectric
                  << " | " << std::setw(7) << components[2]  // Compton  
                  << " | " << std::setw(8) << components[1]  // Coherent
                  << "\n";
    }
    
    std::cout << "\n✅ All photon transport integration tests completed successfully!\n";
