...
int main() {
    ...
    std::cout << "  Reference temperature: " << lead.getReferenceTemperature() << " K\n";
    std::cout << "  Thermal expansion coeff: " << lead.getThermalExpansionCoefficient() * 1e6 << " × 10⁻⁶ K⁻¹\n";
    ...
    std::vector<double> temperatures = {223.15, 273.15, 293.15, 373.15, 473.15, 573.15}; // K
    std::vector<std::string> temp_labels = {"Cold (-50°C)", "Freezing (0°C)", "Room (20°C)", 
                                          "Hot (100°C)", "Very Hot (200°C)", "Extreme (300°C)"};
    
    std::cout << "\nTemperature Effects on Lead:\n";
    std::cout << "Temperature | Density (g/cm³) | Linear μ (cm⁻¹) | Relative Change\n";
    std::cout << "------------|------------------|------------------|-----------------\n";
    
    for(size_t i = 0; i < temperatures.size(); ++i) {
        double temp_K = temperatures[i];
        double corrected_density = lead.getTemperatureCorrectedDensity(temp_K);
        double corrected_atten = lead.getLinearAttenuationCoefficientAtTemperature(energy_kev, temp_K);
        double relative_change = (corrected_atten - ref_linear_atten) / ref_linear_atten * 100.0;
        
        std::cout << std::left << std::setw(12) << temp_labels[i]
                  << "| " << std::setw(16) << corrected_density
                  << " | " << std::setw(16) << corrected_atten
                  << " | " << std::setw(15);
    ...
    double room_atten = aluminum.getLinearAttenuationCoefficientAtTemperature(energy_kev, room_temp);
    double hot_atten = aluminum.getLinearAttenuationCoefficientAtTemperature(energy_kev, hot_temp);
    
    std::cout << "  Aluminum @ 20°C:  ρ = " << room_density << " g/cm³, μ = " << room_atten << " cm⁻¹\n";
    std::cout << "  Aluminum @ 300°C: ρ = " << hot_density << " g/cm³, μ = " << hot_atten << " cm⁻¹\n";
    std::cout << "  Density change: " << (hot_density - room_density) / room_density * 100.0 << "%\n";
    std::cout << "  Attenuation change: " << (hot_atten - room_atten) / room_atten * 100.0 << "%\n";
